...
\begin{tikzpicture}[->,>=stealth']
 ...
 node distance=4cm,     % distance to RPC
 ...
 \begin{tabular}{l}     % content
  \textbf{NetworkControl}\\
  \parbox{2.8cm}{[RPC]\texttt{startGame()}}
 \end{tabular}
 };
 ...
 \begin{tabular}{l}
  ...
  \parbox{2.8cm}{\texttt{onGameStarted()}}
 \end{tabular}
 };
 ...
 \node[state,
 right of=MC,
 node distance=5cm,text width=3cm] (Game) 
 {%
 \begin{tabular}{l}
  \textbf{Game}\\
  \parbox{2.8cm}{\texttt{startNewGame()}}
 \end{tabular}
 };

 % draw the paths and and print some Text below/above the graph
 \path (RPC) edge node {} (NC)
       (NC)  edge node [anchor=south]{event}  (MC)
       (MC)  edge node[anchor=south] {call} (Game)
 ;
\end{tikzpicture}
}
\caption{Client side}
...
\begin{tikzpicture}[->,>=stealth']
 ...
 \begin{tabular}{l}     % content
  \textbf{MainController}\\
  \parbox{2.8cm}{\texttt{startNetwokGame()}}
 \end{tabular}
 };
 ...
 \begin{tabular}{l}
  \textbf{NetworkControl}\\
  \parbox{3.6cm}{\texttt{broadcastNewGame()}
  				[RPC]\texttt{startNewGame()}
 	}
 \end{tabular}
 };
 ...
 \path (RPC) edge node {} (MC)
       (MC)  edge node [anchor=south]{call}  (NC)
       (NC)  edge [loop right, min distance=10mm,in=-7,out=7,looseness=5] node[anchor=west] {RPC} (NC)
